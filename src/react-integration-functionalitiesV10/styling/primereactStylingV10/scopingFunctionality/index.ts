import { PrimeReactScoperOptions } from './types';
import {
  normalizeForHash,
  hash,
  getStyleFromNode,
...
export function attachPrimeReactScoper({
  ...
  const processStyleElement = (
    ...
    if (cssContent && attrName !== 'data-primereact-style-id')
      cssContent = adjustScopeIdToRemote(cssContent);
  ...
  if (bootstrapExisting) {
    ...
    document.head
      .querySelectorAll(
        `style[${attrName}], style[id^="${PRODUCT_NAME}|${APP_NAME}"]`,
      )
