...
describe('createPalette', () => {
  ...
  it('should create a cyan palette with adjusted colors', () => {
    ...
    paletteKeys.forEach((key: number) => {
      console.log(
        '###',
        key,
        colorDelta(palette[key], primeNGCyanColorPalette[key])
      );
      expect(
        colorDelta(palette[key], primeNGCyanColorPalette[key])
  ...
  it('should create a red color palette with adjusted colors', () => {
    ...
    paletteKeys.forEach((key: number) => {
      expect(
        colorDelta(palette[key], primeNGRedColorPalette[key])
      ).toBeLessThanOrEqual(maximumDifference);
    });
  });
  ...
  it('should create an orange color palette with adjusted colors', () => {
    ...
    paletteKeys.forEach((key: number) => {
      expect(
        colorDelta(palette[key], primeNGOrangeColorPalette[key])
      ).toBeLessThanOrEqual(maximumDifference);
    });
  });
  ...
  it('should create a green color palette with adjusted colors', () => {
    ...
    paletteKeys.forEach((key: number) => {
      expect(
        colorDelta(palette[key], primeNGGreenColorPalette[key])
      ).toBeLessThanOrEqual(maximumDifference);
    });
  });
  ...
  it('should create a custom palette', () => {
    ...
    const adjustmentValues: number[] = [
      210, 195, 150, 98, 20, -25, -65, -80, -110, -140,
    ];
    ...
    const exampleColorPalette: ColorPalette = Object.keys(adjustments).reduce(
      (palette, key) => {
        palette[Number(key)] = adjustColor(
          primaryColor,
          adjustments[Number(key)]
        );
        return palette;
      },
      {} as ColorPalette
